...
保持供電，玩家完整入內、非 spectator、關門且全員有 QuantumState，就會自動準備左右延伸的 M2 走廊；成功入場保留全員現有效果與倒數，不需新的 rising edge。任一凍結參與者 Buff 失效則整組返還；仍 HIGH 時保護持續，全員補喝且關門可建立新 SID。斷電先返還完整 cohort 與有價物品，再清理與確認 OFF，最後才解除原艙保護；UUID 與 Registry 碰撞占位保留。RETURNING／UNKNOWN／協調失敗不能先解鎖。
...
2. 單人可等待 Buff 自然到期或喝奶返回；如要測走廊內的外部 LOW，入艙前另安排並量測自行斷電的外部計時器，詳見 [M2 單人流程](m2-corridor.md#單人無遊戲指令流程)。
3. 在室外把 Controller 的外接拉桿／計時供電電路啟動；空艙預期 IDLE／3，但 UUID 已註冊、艙體已保護。
4. 普通右鍵開門，走進 interior，再瞄準入口上方 Controller 關門。
5. 完整站在室內並真飲用藥水，已有有效效果則等待；保持供電且關門便自動準備左右走廊，成功後保留全員 Buff 與自然倒數，人工核對 HUD。
6. 任一位自然到期或喝奶後全組回同一原世界／原艙，HIGH 仍受保護；全員補喝並關門可再入場。另測外部 LOW：完整返還／清理後確認 OFF，才以 Creative 拆除；管理停用不能代替斷電。
...
維持人在 interior：第一個 air 指令讓輸入保持低，完成安全協調後為 INVALID／0；第二個 setblock 供電後，資格齊全會準備並進入 M2 走廊。供電缺資格為 IDLE／3；READY／7 包含準備等待，ARMED／11 可表示活動或等待返還，不能單憑 11 判定安全完成。供電前查 `ChamberUuid` 沒有欄位屬於正常草稿。指令僅供診斷；單人可由 Buff 自然失效返回，另測 LOW 才安排外部定時器，不用刪世界。
...
| C | 保持供電，玩家完整入內、非 spectator、全員有 QuantumState、門已關 | 自動進入左右走廊，保留 Buff／HUD 倒數，不需新 rising edge |
...
| I | 觀察上述全程 | 入場保留 Buff；任一位失效全組返還，HIGH保持保護與補喝再入；LOW安全收尾才可拆；不新增 Universe |
