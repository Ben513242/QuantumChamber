...
本指引對應 `feature/m1-chamber` 的 M2：有效原艙（Origin）由外部供電註冊／保護，再讓單人入內關門、補齊藥水，成功後消耗入場效果並進入真走廊。施工尺寸與材料沿用 M1；這是操作指引與待驗清單，不是已通過人工驗收的證據。完整單人計時斷電與 M2 恢復界線見 [M2 操作與驗證](m2-corridor.md)。
...
保持供電，玩家完整入內、非 spectator、關門且全員有 QuantumState，就會自動準備 M2 走廊，成功入場後一次消耗全員效果，無須再到室外製造新的 rising edge。啟動前缺資格會繼續等待；已成功的活動 session 不以藥效消失取消，因為入場本來就會消耗效果。斷電先返還完整 cohort 與有價物品，再清理與確認 OFF，最後才解除原艙保護；UUID 與 Registry 碰撞占位仍保留。RETURNING／UNKNOWN／協調失敗不能先解鎖。室內裝置請在供電前規劃。
...
2. 入艙前先確認外部計時電路已安排自行斷電；單人進入固定走廊世界後不能留在原世界撥桿。先在空艙旁測好倒數時間，實作範例見 [M2 單人流程](m2-corridor.md#單人無遊戲指令流程)。
3. 在室外把 Controller 的外接拉桿／計時供電電路啟動；空艙預期 IDLE／3，但 UUID 已註冊、艙體已保護。
4. 普通右鍵開門，走進 interior，再瞄準入口上方 Controller 關門。
5. 完整站在室內並飲用藥水，已有有效效果則等待即可；保持供電且門已關閉便自動準備走廊，成功後全員入場並消耗效果。
6. 等外部計時器斷電，確認回到同一原世界／原艙；全 cohort 與有價物品安全返還、session 與租約清理完成後才解除保護。最後 OFF 才以 Creative 拆除；不要以管理停用取代斷電。
...
維持人在 interior：第一個 air 指令讓輸入保持低，完成安全協調後為 INVALID／0；第二個 setblock 供電後，資格齊全會準備並進入 M2 走廊。供電缺資格為 IDLE／3；READY／7 包含準備等待，ARMED／11 可表示活動或等待返還，不能單憑 11 判定安全完成。供電前查 `ChamberUuid` 沒有欄位屬於正常草稿。上述指令僅供選用診斷；單人無指令流程須事先接好外部定時斷電，不用刪世界。
...
| C | 保持供電，玩家完整入內、非 spectator、全員有 QuantumState、門已關 | 自動準備並進入真走廊、一次消耗效果，不需新的 rising edge |
| D | 入場前保持門開，喝牛奶移除效果；關門後仍缺效果，再補喝 QuantumState | 缺效果不啟動，補齊後才入場；供電保護持續 |
...
| I | 觀察上述全程 | 真走廊入場與 buff 消耗；斷電回同一原艙，完整返還後才可拆；不新增動態 Dimension／Universe |
...
5. 未供電且安全協調完成時為 INVALID／0；保持供電、全員完整入內且關門有 buff，會自動準備與進入 M2 走廊。準備期間 READY／7，活動與等待返還都可能是 ARMED／11；看見數值不能替代真世界／原艙位置與拆除安全驗收。
