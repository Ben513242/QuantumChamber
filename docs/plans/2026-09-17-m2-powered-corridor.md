...
- [x] **Step 5: commit／review／handoff。** `test: 完成新版 M2 持久化 gate 與驗收指引`，只有本task獨立review；wholebranchreview仍在後續整體收尾一次，所有Minor/rulings/CI證據需交接。M3開始前文件明列沒有候選選擇/塌縮/family全量/passage。main不merge／無LICENSE／不刪owner或userworld直到所有必要gate明確關閉。

2026-09-19 Task10完整自動gate：固定code `4d7c2dfe93a59ba0278039545b4555b7a7747e35`，native舊ARMING,true期望RED保留→新mode/KEEP_CURRENT GREEN；最後36個原生phase（34個數字PID、36個不同StartTime/startupNonce）含四schema1 legacy與新W1／W2／舊RESTORE中斷及二次恢復，31組正式NBT讀回成功。313份source/class指紋×36phase全對上最後clean classes。唯一末次非快取fullgate19工作全執行，183JUnit declared／182通過／1合法NonWindows skip、必要Windows12零skip、fresh GT116零failure/error/skip；雙JAR41 testmod class／25 source交集與污染0、main-client引用0、main-only四世界保存／exit0、照明SHA符合且未啟動GPU。獨立Spec✅／qualityApproved、0Critical／0Important／3Minor；Cannot verify已逐項歸屬，W2 timeout實作拒絕但未另跑到期分支，不冒稱動態覆蓋。新精確SHA[run35369672563](https://github.com/Ben513242/QuantumChamber/actions/runs/35369672563) Windows/Ubuntu皆success，Windowsbuild/GT與必要guard真success，原XML必要12零skip與116GT全零；UbuntuNonWindows拒絕實跑綠、25平台skip不冒稱native。所有原warnings／runtime noise與Minor留finaltriage。M2必要自動與逐task gate已關閉，可進已授權M3 mandatory feasibility spike；8人工／wholebranch仍pending、M3尚未執行、main未合併。
